...
export default function Header({ onCartOpen, customPages = [] }: { onCartOpen: () => void, customPages?: CustomPageLink[] }) {
        ...
        >
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-[70px] flex items-center justify-between gap-4">
                ...
                <Link href="/" className="flex items-center gap-2 shrink-0 group">
                    ...
                    <span className="font-serif text-xl font-bold text-[#2D2D2D] tracking-tight leading-none">
                        Nette&apos;s<span className="text-[#7D8E74]"> Cafe</span>
                    </span>
                </Link>
                ...
                <div className="md:hidden bg-[#F9F7F2]/95 backdrop-blur-xl border-t border-[#7D8E74]/15 px-4 pb-5 pt-3 space-y-1">
                    {NAV.map(({ href, label }) => (
                        <Link
                            ...
                            className={`block px-4 py-3 rounded-2xl font-medium text-sm transition ${pathname === href
                                ? 'bg-[#7D8E74]/15 text-[#5a6b52] font-semibold'
                                : 'text-[#2D2D2D]/70 hover:bg-[#7D8E74]/10'
                                }`}
                        >
                            {label}
                        </Link>
                    ...
                    {customPages.map(page => (
                        <Link
                            ...
                            href={`/${page.slug}`}
                            onClick={() => setMobileOpen(false)}
                            className={`block px-4 py-3 rounded-2xl font-medium text-sm transition ${pathname === `/${page.slug}`
                                ? 'bg-[#7D8E74]/15 text-[#5a6b52] font-semibold'
                                : 'text-[#2D2D2D]/70 hover:bg-[#7D8E74]/10'
                                }`}
                        >
                    ...
                    <Link
                        href="/contact"
                        onClick={() => setMobileOpen(false)}
                        className="flex items-center gap-2 mt-2 px-4 py-3 rounded-2xl bg-[#7D8E74] text-white font-semibold text-sm"
                    >
